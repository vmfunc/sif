...
func TestLLMInferenceExposureModules(t *testing.T) {
	...
	t.Run("a vllm models api is flagged with its model id", func(t *testing.T) {
		res := runInferenceExposureModule(t, vllm, 200, vllmModels)
		if len(res.Findings) == 0 {
			t.Fatal("expected a vllm finding")
		}
		if v := inferenceExtract(res, "vllm_model"); v != "meta-llama/Llama-3.1-8B-Instruct" {
			t.Errorf("vllm_model=%q, want meta-llama/Llama-3.1-8B-Instruct", v)
		}
	})

	t.Run("a vllm rust frontend list is still flagged", func(t *testing.T) {
		...
		if res := runInferenceExposureModule(t, vllm, 200, body); len(res.Findings) == 0 {
			t.Fatal("expected the vllm rust frontend (owned_by vllm-frontend-rs) to be flagged")
		}
	})

	t.Run("a llamacpp models api is flagged with its model id", func(t *testing.T) {
		res := runInferenceExposureModule(t, llamacpp, 200, llamacppModels)
		if len(res.Findings) == 0 {
			t.Fatal("expected a llamacpp finding")
		}
		if v := inferenceExtract(res, "llamacpp_model"); v != "/models/llama-2-7b.Q4_K_M.gguf" {
	...
	t.Run("a llamacpp models api is not flagged as vllm", func(t *testing.T) {
		if res := runInferenceExposureModule(t, vllm, 200, llamacppModels); len(res.Findings) > 0 {
	...
	t.Run("a vllm models api is not flagged as llamacpp", func(t *testing.T) {
		if res := runInferenceExposureModule(t, llamacpp, 200, vllmModels); len(res.Findings) > 0 {
	...
	t.Run("an infinity models list is flagged with its model id", func(t *testing.T) {
		res := runInferenceExposureModule(t, infinity, 200, infinityModels)
		if len(res.Findings) == 0 {
			t.Fatal("expected an infinity finding")
		}
		if v := inferenceExtract(res, "infinity_model"); v != "BAAI/bge-small-en-v1.5" {
	...
	t.Run("an infinity list without owned_by is not flagged", func(t *testing.T) {
		...
		if res := runInferenceExposureModule(t, infinity, 200, body); len(res.Findings) > 0 {
	...
	t.Run("a vllm models list is not flagged as infinity", func(t *testing.T) {
		if res := runInferenceExposureModule(t, infinity, 200, vllmModels); len(res.Findings) > 0 {
	...
	t.Run("an infinity list is not flagged as vllm", func(t *testing.T) {
		if res := runInferenceExposureModule(t, vllm, 200, infinityModels); len(res.Findings) > 0 {
	...
	t.Run("an lmstudio models api is flagged with its model id", func(t *testing.T) {
		res := runInferenceExposureModule(t, lmstudio, 200, lmstudioModels)
		if len(res.Findings) == 0 {
			t.Fatal("expected an lmstudio finding")
		}
		if v := inferenceExtract(res, "lmstudio_model"); v != "qwen2-vl-7b-instruct" {
	...
	t.Run("a body without compatibility_type is not flagged as lmstudio", func(t *testing.T) {
		...
		if res := runInferenceExposureModule(t, lmstudio, 200, body); len(res.Findings) > 0 {
	...
	t.Run("an openai compatible v1 models list is not flagged as lmstudio", func(t *testing.T) {
		if res := runInferenceExposureModule(t, lmstudio, 200, vllmModels); len(res.Findings) > 0 {
	...
	t.Run("an lmstudio models list is not flagged as vllm", func(t *testing.T) {
		if res := runInferenceExposureModule(t, vllm, 200, lmstudioModels); len(res.Findings) > 0 {
